...
/// Benchmark tests for Docker CLI compatibility via Meadow's Docker API socket.
/// These tests verify that standard Docker CLI commands work through
/// Meadow's Unix socket at ~/.meadow/docker.sock using curl as the transport.
///
/// Run with: MEADOW_RUN_DOCKER_API=1 swift test --filter DockerCLIBenchmarkTests
final class DockerCLIBenchmarkTests: XCTestCase {

    private let socketPath = "\(NSHomeDirectory())/.meadow/docker.sock"
    private let testContainerName = "meadow-bench-cli-test"

    override func setUp() async throws {
        guard ProcessInfo.processInfo.environment["MEADOW_RUN_DOCKER_API"] == "1" else {
            throw XCTSkip("Set MEADOW_RUN_DOCKER_API=1 to run Docker CLI benchmark tests")
        }
        ...
        guard pingCode == 0 else {
            throw XCTSkip("Docker API socket exists but is not responding (Meadow Docker API server may not be running)")
        }
    ...
    override func tearDown() async throws {
        ...
        if ProcessInfo.processInfo.environment["MEADOW_RUN_DOCKER_API"] == "1" {
    ...
    func testDockerLogs() throws {
        ...
        let createBody = """
        {"Image":"alpine:latest","Cmd":["sh","-c","echo hello-from-meadow && sleep 30"]}
        """
        try curlSocket(
            "http://localhost/containers/create?name=\(testContainerName)",
            method: "POST",
        ...
        XCTAssertTrue(output.contains("hello-from-meadow"), "logs should contain expected output, got: \(output.prefix(200))")
    ...
    func testDockerVolumeLS() throws {
        ...
        // Docker API returns {"Volumes": [...], "Warnings": [...]}, but Meadow may vary
    ...
    func testDockerInfo() throws {
        ...
        // Meadow may not populate all Docker info fields; just verify it's valid JSON
    ...
    func testDockerVersion() throws {
        ...
        // Meadow may not populate all Docker version fields; just verify it's valid JSON
