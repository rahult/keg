...
final class MeadowE2ETests: XCTestCase {

    override func setUp() async throws {
        guard ProcessInfo.processInfo.environment["MEADOW_RUN_CONTAINER_E2E"] == "1" else {
            throw XCTSkip("Set MEADOW_RUN_CONTAINER_E2E=1 to run container-backed end-to-end tests")
        }
    ...
    func testRunAndManageContainer() async throws {
        ...
        let pipe = Pipe()
        process.executableURL = URL(filePath: "/usr/bin/env")
        process.arguments = ["container", "run", "-d", "--name", "meadow-e2e-test", "alpine:latest", "sleep", "60"]
        process.standardOutput = pipe
        process.standardError = pipe
        try process.run()
        process.waitUntilExit()
        ...
        let container = try await client.get(id: "meadow-e2e-test")
        ...
        // Get stats
        let stats = try await client.stats(id: "meadow-e2e-test")
        ...
        // Stop
        try await client.stop(id: "meadow-e2e-test")
        print("✅ Stopped container")

        // Delete
        try await client.delete(id: "meadow-e2e-test", force: true)
        print("✅ Deleted container")
    }

    func testVolumeCRUD() async throws {
        // Create
        let vol = try await ClientVolume.create(name: "meadow-test-vol")
        XCTAssertEqual(vol.name, "meadow-test-vol")
        print("✅ Created volume: \(vol.name)")

        // List
        let volumes = try await ClientVolume.list()
        XCTAssertTrue(volumes.contains { $0.name == "meadow-test-vol" })
        print("✅ Volume appears in list")

        // Delete
        try await ClientVolume.delete(name: "meadow-test-vol")
