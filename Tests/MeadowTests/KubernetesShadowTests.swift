...
/// Shadow tests for Kubernetes cluster operations via Meadow.
...
///
/// Run with: MEADOW_RUN_K8S_E2E=1 swift test --filter KubernetesShadowTests
final class KubernetesShadowTests: XCTestCase {

    private let clusterName = "meadow-k8s-shadow"
    private var kubeconfigPath: String { NSHomeDirectory() + "/.meadow/kubeconfig-shadow" }

    override func setUp() async throws {
        guard ProcessInfo.processInfo.environment["MEADOW_RUN_K8S_E2E"] == "1" else {
            throw XCTSkip("Set MEADOW_RUN_K8S_E2E=1 to run Kubernetes shadow tests (takes ~2-3 min, needs ~8GB RAM)")
        }
    ...
    override func tearDown() async throws {
        ...
        if ProcessInfo.processInfo.environment["MEADOW_RUN_K8S_E2E"] == "1" {
            await deleteCluster()
        }
    ...
    func testRunJob() async throws {
        ...
        XCTAssertTrue(logOut.contains("hello-from-meadow-k8s"), "Job output should contain our message")
    ...
    func testConfigMapMount() async throws {
        ...
        XCTAssertTrue(getOut.contains("app.name: meadow"))
    ...
    func testCreateSecret() async throws {
        ...
        let (createCode, createOut) = try kubectl(
            ...
            "--from-literal=password=meadow123"
        )
