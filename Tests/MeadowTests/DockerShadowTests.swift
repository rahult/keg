...
/// verifying that Meadow's container platform works end-to-end with practical examples.
///
/// Run with: MEADOW_RUN_CONTAINER_E2E=1 swift test --filter DockerShadowTests
final class DockerShadowTests: XCTestCase {

    private let testPrefix = "meadow-shadow"

    override func setUp() async throws {
        guard ProcessInfo.processInfo.environment["MEADOW_RUN_CONTAINER_E2E"] == "1" else {
            throw XCTSkip("Set MEADOW_RUN_CONTAINER_E2E=1 to run container-backed shadow tests")
        }
    ...
    func testContainerWithEnvironmentVariables() async throws {
        ...
        let (code, output) = try runProcess([
            "container", "run",
            "--name", name,
            "-e", "MY_VAR=hello_meadow",
            "-e", "DB_HOST=localhost",
            "alpine:latest",
            "env"
        ])
        XCTAssertEqual(code, 0, "env command should succeed: \(output)")
        XCTAssertTrue(output.contains("MY_VAR=hello_meadow"), "MY_VAR should be set")
    ...
    func testContainerExec() async throws {
        ...
        let (writeCode, _) = try runProcess([
            "container", "exec", name,
            "sh", "-c", "echo 'meadow-test' > /tmp/meadow-test.txt"
        ])
        XCTAssertEqual(writeCode, 0)

        let (readCode, readOutput) = try runProcess([
            "container", "exec", name,
            "cat", "/tmp/meadow-test.txt"
        ])
        XCTAssertEqual(readCode, 0)
        XCTAssertTrue(readOutput.contains("meadow-test"), "file content should match")
    ...
    func testVolumePersistence() async throws {
        ...
        let (writeCode, writeOut) = try runProcess([
            ...
            "-v", "\(volName):/data",
            "alpine:latest",
            "sh", "-c", "echo 'persistent-data-from-meadow' > /data/test.txt"
        ])
        ...
        XCTAssertTrue(readOut.contains("persistent-data-from-meadow"), "data should persist across containers")
    ...
    func testDockerAPIHealth() async throws {
        guard ProcessInfo.processInfo.environment["MEADOW_RUN_DOCKER_API"] == "1" else {
            throw XCTSkip("Set MEADOW_RUN_DOCKER_API=1 to run Docker API tests (requires Meadow app running with Docker API enabled)")
        }

        let socketPath = "\(NSHomeDirectory())/.meadow/docker.sock"
    ...
    func testDockerAPIListContainers() async throws {
        guard ProcessInfo.processInfo.environment["MEADOW_RUN_DOCKER_API"] == "1" else {
            throw XCTSkip("Set MEADOW_RUN_DOCKER_API=1 to run Docker API tests")
        }
