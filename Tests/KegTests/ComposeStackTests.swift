...
/// Real-world multi-service Docker Compose stack tests via Keg's ComposeOrchestrator.
...
///
/// Run with: KEG_RUN_CONTAINER_E2E=1 swift test --filter ComposeStackTests
final class ComposeStackTests: XCTestCase {

    private let testPrefix = "keg-compose-stack"
    ...
    override func setUp() async throws {
        guard ProcessInfo.processInfo.environment["KEG_RUN_CONTAINER_E2E"] == "1" else {
            throw XCTSkip("Set KEG_RUN_CONTAINER_E2E=1 to run compose stack tests")
        }
        try await super.setUp()
        try await ensureSystemRunning()

        tempDir = FileManager.default.temporaryDirectory.appendingPathComponent("keg-stack-\(UUID().uuidString.prefix(8))")
        ...
        services:
          ...
          api:
            ...
            depends_on:
              - db
            environment:
              DATABASE_URL: postgres://postgres:testpass@db:5432/kegtest
        """
