...
//
// Gate: KEG_RUN_DOCKER_API=1
// Run:  KEG_RUN_DOCKER_API=1 swift test --filter DockerAPIContractTests
...
final class DockerAPIContractTests: XCTestCase {
    ...
    override func setUp() async throws {
        guard ProcessInfo.processInfo.environment["KEG_RUN_DOCKER_API"] == "1" else {
            throw XCTSkip("Set KEG_RUN_DOCKER_API=1 to run Docker API contract tests")
        }
    ...
    func testCreateContainer() throws {
        ...
        """
        let (status, body) = try curlSocket("POST", "/containers/create?name=keg-contract-test", body: createBody)
