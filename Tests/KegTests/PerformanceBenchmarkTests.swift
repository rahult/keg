...
/// Performance benchmark tests for Keg container operations.
...
///
/// Run with: KEG_RUN_CONTAINER_E2E=1 swift test --filter PerformanceBenchmarkTests
final class PerformanceBenchmarkTests: XCTestCase {

    private let testPrefix = "keg-bench"

    override func setUp() async throws {
        guard ProcessInfo.processInfo.environment["KEG_RUN_CONTAINER_E2E"] == "1" else {
            throw XCTSkip("Set KEG_RUN_CONTAINER_E2E=1 to run performance benchmark tests")
        }
    ...
    func testBenchSystemIdleMemory() async throws {
        ...
        if output.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            print("  [BENCH] No Keg/container runtime processes found - skipping RSS measurement")
    ...
    /// Measures RSS of container-apiserver, container-core-images, container-network-vmnet, and Keg.app.
