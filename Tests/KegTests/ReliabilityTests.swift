...
/// Reliability and edge-case tests for Keg container operations.
/// These tests verify that Keg handles error conditions gracefully without crashing.
///
/// Run with: KEG_RUN_CONTAINER_E2E=1 swift test --filter ReliabilityTests
final class ReliabilityTests: XCTestCase {

    private let testPrefix = "keg-rel"

    override func setUp() async throws {
        guard ProcessInfo.processInfo.environment["KEG_RUN_CONTAINER_E2E"] == "1" else {
            throw XCTSkip("Set KEG_RUN_CONTAINER_E2E=1 to run container-backed reliability tests")
        }
