...
/// Shadow tests for Docker Compose operations via Keg's ComposeOrchestrator.
...
///
/// Run with: KEG_RUN_CONTAINER_E2E=1 swift test --filter ComposeShadowTests
final class ComposeShadowTests: XCTestCase {

    private let testPrefix = "keg-compose-shadow"
    ...
    override func setUp() async throws {
        guard ProcessInfo.processInfo.environment["KEG_RUN_CONTAINER_E2E"] == "1" else {
            throw XCTSkip("Set KEG_RUN_CONTAINER_E2E=1 to run compose shadow tests")
        }
        try await super.setUp()
        try await ensureSystemRunning()

        // Create temp directory for compose files
        tempDir = FileManager.default.temporaryDirectory.appendingPathComponent("keg-compose-\(UUID().uuidString.prefix(8))")
