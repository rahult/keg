...
struct TerminalView: View {
    ...
    private func startShell() {
        ...
        do {
            ...
            inputPipe = inPipe
            isRunning = true
            output += "Meadow Terminal — shell ready\n\n"
        } catch {
...
private enum TerminalPreset: String, Identifiable {
    ...
    var command: String? {
        ...
        case .shell:
            return nil
        case .dockerPS:
            return "export DOCKER_HOST=unix://\(NSHomeDirectory())/.meadow/docker.sock && docker ps -a"
        case .containerList:
            return "container list -a"
        case .containerImages:
            return "container image list"
        case .kubectlPods:
            return "export KUBECONFIG=\"\(NSHomeDirectory())/.meadow/kubeconfig\" && kubectl get pods -A"
        }
    }
}
