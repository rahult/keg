...
struct ContainerListView: View {
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .meadowRunContainer)) { _ in
            showRunSheet = true
        }
        .onReceive(NotificationCenter.default.publisher(for: .meadowRefresh)) { _ in
            Task { await vm.refresh() }
        }
        .onReceive(NotificationCenter.default.publisher(for: .meadowStopContainer)) { _ in
            guard !selectedContainerIDs.isEmpty else { return }
            Task { for id in selectedContainerIDs { await vm.stop(id: id) } }
        }
        .onReceive(NotificationCenter.default.publisher(for: .meadowDeleteContainer)) { _ in
            guard !selectedContainerIDs.isEmpty else { return }
            showingDeleteConfirmation = true
        }
        .onReceive(NotificationCenter.default.publisher(for: .meadowFocusSearch)) { _ in
            guard appState.selectedMeadowSection == .containers else { return }
