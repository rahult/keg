...
actor WebhookManager {
    ...
    private func triggerWebhook(_ webhook: Webhook, event: WebhookEvent, container: DockerContainer?, image: DockerImage?) async {
        ...
        request.setValue("Meadow-Webhook/1.0", forHTTPHeaderField: "User-Agent")
        ...
        if let secret = webhook.secret {
            do {
                ...
                request.setValue("sha256=\(signature)", forHTTPHeaderField: "X-Meadow-Signature")
...
final class DockerAPIServer: Sendable {
    ...
    /// Try binding to `/var/run/docker.sock` first; fall back to `~/.meadow/docker.sock`.
    func start() async throws {
        // Ensure ~/.meadow directory exists
        let meadowDir = URL(filePath: NSHomeDirectory()).appendingPathComponent(".meadow")
        try FileManager.default.createDirectory(at: meadowDir, withIntermediateDirectories: true)
